...
export function FocusPane() {
  ...
  return (
    <div className="w-[44%] border-l border-[--color-border] bg-[--color-canvas] flex flex-col min-w-0">
      <div className="h-11 px-4 flex items-center justify-between border-b border-[--color-border] shrink-0">
        <div className="flex items-center gap-2 text-[11px] font-[--font-mono] text-[--color-muted]">
          <span>trace</span>
          <span className="text-[--color-text]">{cell.id.slice(-10)}</span>
        </div>
        <div className="flex gap-1">
          <button
            ...
            className="px-2 py-1 text-[11px] border border-[--color-border] rounded
                       hover:bg-[--color-surface] transition flex items-center gap-1"
          >
          ...
          <button
            onClick={() => focus(null)}
            aria-label="Close"
            className="p-1 text-[--color-muted] hover:text-[--color-text] rounded hover:bg-[--color-surface]"
          >
            <X className="w-4 h-4" />
          </button>
        </div>
      ...
      <div className="p-4 space-y-5 overflow-auto">
        <section>
          <div className="text-[10px] text-[--color-muted] uppercase tracking-wide mb-1">
            Answer
          </div>
          <div className="text-xl font-[--font-ui] min-h-[1.75rem]">
            {cell.status === "done" ? (
              <CellRenderer cell={cell} />
            ) : (
              <span className="text-[--color-muted] text-[13px]">{cell.status}…</span>
            )}
          </div>
          <div className="mt-2 flex items-center gap-3 text-[11px] text-[--color-muted] font-[--font-mono]">
        ...
        {citations.length > 0 && (
          <section>
            <div className="text-[10px] text-[--color-muted] uppercase tracking-wide mb-2">
            ...
            <div className="flex flex-wrap gap-1.5">
              {citations.map((c, i) => (
                <button
                  ...
                  className={cn(
                    "px-2 py-1 text-[11px] font-[--font-mono] rounded border transition",
                    i === activeCite
                      ? "border-[--color-accent-streaming] text-[--color-accent-streaming] bg-[--color-accent-streaming]/10"
            ...
            {cite && (
              <div className="mt-3 text-[12px] bg-[--color-surface] p-3 rounded border border-[--color-border] leading-relaxed">
                {cite.snippet}
              </div>
            )}
          </section>
        ...
        <section>
          <button
            ...
            className="flex items-center gap-1 text-[10px] uppercase tracking-wide text-[--color-muted] hover:text-[--color-text]"
          >
        ...
        {cell.error && (
          <section className="p-3 rounded border border-[--color-accent-fail]/40 bg-[--color-accent-fail]/10 text-[12px] text-[--color-accent-fail] font-[--font-mono]">
            {cell.error}
          </section>
        )}
      </div>
...
function TraceView({ cellId }: { cellId: string }) {
  ...
  return (
    <pre className="mt-2 text-[11px] font-[--font-mono] text-[--color-muted] whitespace-pre-wrap leading-relaxed">
